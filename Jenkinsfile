    ...
    node('mypod') 
    {
        ...
        stage ('Docker build && push ') 
        {
            container ('docker') 
            {
                ...
                sh "echo $registryIp"
                repository = "ahmedcheibani/fraud_api_k8s"
        ...
        stage ('Helm Deploy Application to Kubernates')
        {
            container ('helm') 
            {
                sh "helm upgrade apifraud fraudapp-chart -n fraude-Dev-env -i --wait --set image.repository=${repository},image.tag=${commitId}"
            }
        }
    }   
}
