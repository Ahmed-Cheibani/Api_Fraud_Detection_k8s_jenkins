...
def predict_transaction():
    if(request.method=='POST'):
        ...
        prepared_trnasaction  = fraud_detection.prepare_data(transaction)
        return fraud_detection.prediction(original_transaction, prepared_trnasaction)
...
def predict_transactions():
    if(request.method=='POST'):
        ...
        for elem in transactions:
            original_elem = transactions.copy()
            prepared_elem = fraud_detection.prepare_data(elem)
            predictions.append(fraud_detection.prediction(original_elem, prepared_elem))
